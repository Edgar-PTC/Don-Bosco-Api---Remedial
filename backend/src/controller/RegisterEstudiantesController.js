...
registerEstudiantes.insertEstudiante = async (req, res) => {
    try {
        let { name, lastName, email, password, career } = req.body;
        ...
        if(!name || !lastName || !email || !password || !career){
        ...
        const emailExist = await Estudiantes.findOne({email})
        if(emailExist){
        ...
        const newEstudent = await Estudiantes({ name, lastName, email, password: passwordHash, career, isVerified: false, loginAttemps: 0 });
        await newEstudent.save();
...
registerEstudiantes.verifyCode = async (req, res) => {
    try {
        ...
        const Estudiante = await Estudiantes.findOne({email});
